...
create table devices (
  ...
  access_kind text not null default 'primary' check (access_kind in ('primary', 'extra_session')),
...
-- A computer remains permanently tied to one branch because
-- device_fingerprint itself is globally unique. Multiple computers may open
-- the same branch when an admin shares the branch extra-session PIN.
...
create table device_access_pins (
...
-- DEVICES: branch computers request access through fn_request_branch_device.
-- SELECT is limited to admin or the row matching own fingerprint so a device
-- can poll its own approval/PIN status without seeing other computers.
